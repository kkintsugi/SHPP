 ...
 * a two-dimensional {@code SmartPixel} array, and uses the recursive Depth-First Search (DFS) algorithm
 ...
 */
public class Assignment11Part1V2WithRecursion {
     ...
     */
    private static final double MIN_SILHOUETTE_SCALE = 0.3;
    ...
    /**
     * Gets two-dimensional array of integers, which stores the RGB values of the pixels in the image.
     *
    ...
    /**
     * Explores a single silhouette starting from the given pixel using recursive DFS algorithm.
     * Keeps track of the maximum and minimum coordinates of the silhouette.
     * Extreme points of the silhouettes are placed in array this way:
    ...
    private SilhouetteCharacteristics exploreSilhouette(SmartPixel[][] smartPixelArray, SmartPixel startPixel) {
        ...
        dfs(smartPixelArray, startPixel, extremePoints);

        return new SilhouetteCharacteristics(extremePoints[0], extremePoints[1], extremePoints[2], extremePoints[3]);
    }

    /**
     * Recursive method for DFS algorithm.
     *
     * @param smartPixelArray The 2D array of SmartPixels.
     * @param currentPixel    The current pixel being visited.
     * @param extremePoints   An array storing minRow, maxRow, minCol, maxCol of the silhouette.
     */
    private void dfs(SmartPixel[][] smartPixelArray, SmartPixel currentPixel, int[] extremePoints) {
        currentPixel.setVisited();

        int row = currentPixel.getRow();
        int col = currentPixel.getCol();

        if (row < extremePoints[0]) {
            extremePoints[0] = row;
        }
        if (row > extremePoints[1]) {
            extremePoints[1] = row;
        }
        if (col < extremePoints[2]) {
            extremePoints[2] = col;
        }
        if (col > extremePoints[3]) {
            extremePoints[3] = col;
        }
        ...
        for (int i = 0; i < NUM_NEIGHBORS; i++) {
            ...
            if (isRowAndColValid(neighborRow, neighborCol, smartPixelArray)) {
                SmartPixel neighbor = smartPixelArray[neighborRow][neighborCol];
                if (!neighbor.isBackground() && !neighbor.isVisited()) {
                    dfs(smartPixelArray, neighbor, extremePoints);
                }
            }
        }
    }
